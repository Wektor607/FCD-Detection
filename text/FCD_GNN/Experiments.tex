...
\begin{itemize}
    \item \textbf{MELD.} The pretrained \ac{meld} model was evaluated on the test set without any additional training or fine-tuning and is used as the baseline.
    ...
    As in Exp1, multi--scale visual features are extracted from the pretrained \ac{meld} decoder and used as input to the \texttt{GuideDecoder}.
    ...
    The upsampling path and segmentation head are kept identical to Exp1.
    ...
    As in Exp1--Exp2, multi--scale visual features are extracted from the pretrained \ac{meld} decoder and used as input to the \texttt{GuideDecoder}.
    In contrast to Exp2, the full \texttt{GuideDecoder} is enabled, i.e., self--attention on image tokens and cross--attention between image and text embeddings are applied (see Fig.~\ref{fig:basic_model}).
    ...
    \begin{table}[t]
    \centering
    \caption{Text conditioning variants used in Exp3.}
    ...
    \renewcommand{\arraystretch}{1.15}
        \begin{tabularx}{\textwidth}{l >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
            ...
            Left Middle Frontal Gyrus; Precentral Gyrus; \dots \\
            \midrule
            hemi+lobe\_regions & Hemisphere + lobe regions &
            Left hemisphere; Middle Frontal Gyrus; \dots \\
            \midrule
            hemi+lobe & Hemisphere + lobe name & Left hemisphere; Frontal lobe \\
            \midrule
    ...
    Same as \textbf{Exp3}, but Atlas descriptions were randomly sampled from the text conditioning variants listed in Table~\ref{tab:text_variants}.
...
\begin{table}[H]
    ...
    \item \textit{Exp3 prompt variants:}
    ...
    \item \textbf{Exp1} and \textbf{Exp2} are ablation baselines.
    \end{tablenotes}
...
Across all \textit{Exp3 models}, specificity was $\sim$36--42 percentage points higher than that of MELD. 
...
Across all \textit{Exp3 models}, specificity was higher than that of MELD by approximately 36--42 percentage points.
This indicates that the text--conditioned models distinguish healthy from non--healthy cases substantially better.
...
As shown in Table~\ref{tab:main_cohort}, the median PPV$_{\text{clusters}}$ attains high values for many configurations (often reaching 0.667, with overlapping confidence intervals), which leads to a ceiling effect and provides limited discrimination between models.
...
MELD achieves the highest mean PPV$_{\text{clusters}}$ on the main cohort, with a small margin of approximately 2--3 percentage points over the next two models and a much larger gap to the remaining ones.
On the independent cohort (Table~\ref{tab:independent_cohort}), however, the situation is reversed: the proposed architecture shows improvements of 2--8 percentage points compared with the two best-performing baselines.
...
80.5\% vs.\ 76.8\%). However, this came at 
...
\textit{hemi + lobe + PubMedBERT} model achived the highest sensitivity ($66/82$), a 
...
Each figure shows (from left to right) the ground-truth lesion mask, the prediction of the baseline MELD model, and the prediction of the proposed Exp3 model with PubMedBERT using the \texttt{hemi+lobe} prompt.
...
Finally, we present an example comparing the same Exp3 model conditioned on different textual descriptions, namely the \textit{hemi} and \textit{hemi + lobe} prompts.
\begin{figure}[H]
    \centering
...
Based on Tables~\ref{tab:main_cohort_mixed_correct} and \ref{tab:main_cohort_mixed_5_epochs_correct}, introducing a 5-epoch warm--up increases specificity by approximately 13.6 percentage points and sensitivity by about 4 percentage points on average, while Dice, PPV, and IoU decrease slightly (by about 1--2 percentage points).
...
Using the \texttt{no\_text} setting (i.e., providing a fixed placeholder prompt, \textit{full brain}, for both patients and healthy controls) does not substantially degrade overall performance, but the metrics become consistently worse.
Compared to evaluation with correct descriptions, \textbf{Specificity} decreases by roughly 27 percentage points.
A similar pattern is observed for \textbf{sensitivity}: the \texttt{no\_text} variant detects fewer lesions, with sensitivity lower by about 1--2 percentage points compared to using wrong prompts, and by about 6--7 percentage points compared to using correct prompts.
...
Based on the results from the previous sections, we select the final experimental setup as follows:
\begin{enumerate}[label=(\roman*), leftmargin=*]
\item the text encoder is RadBERT (used throughout all experiments in this work),
\item the visual branch uses GNN encoders with 3 and 5 blocks, and
\item for each GNN depth, we initialize the model from the corresponding best--performing Exp1+GNN checkpoint and employ a 5-epoch decoder warm--up, after which the full decoder is unfrozen.
\end{enumerate}
...
\item[] \textit{Note.}
\item \textit{Basic Exp1} denotes the pretrained Exp1 model without GNN blocks; \textit{Exp1 with 3/5 GNN} denotes the pretrained Exp1 model with 3/5 GNN blocks.
\item Dice, IoU, and PPV metrics are computed on patients only (subjects with lesions).
\item All experiments in this table use the \textit{Exp3\_mixed} training scheme to obtain results more efficiently.
\end{tablenotes}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Exp3\_hemi+lobe+PubMedBERT} achieves the highest \textbf{Sensitivity} and \textbf{Specificity} on both cohorts. 
