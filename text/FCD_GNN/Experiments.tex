...
Across all \textit{Vision+LM models}, specificity was higher than that of MELD by $\sim$36--42 percentage points.
...
MELD achieves the highest mean PPV$_{\text{clusters}}$ on the main cohort, with a small margin of $\sim$2--3 percentage points over the next two models and a much larger gap to the remaining ones.
...
$\sim$1--2\% fewer lesions relative to models using wrong prompts, 
...
$\sim$5\% more lesions (higher \textbf{Sensitivity}).  
...
Dice and IoU improve by $\sim$2-3\%, and Sensitivity increases by $\sim$2-4\% compared to
...
by $\sim$3-7\% in Dice and IoU, although with a slight decrease in Sensitivity (typically 1-2\%).
...
Connecting \textbf{all seven} MELD stages to the GNN yields the highest lesion count, with sensitivity increasing by $\sim$3.5 percentage points (\(188/259\) vs.\ \(179/259\)).
This improvement comes with only a modest reduction in Dice and PPV\textsubscript{pixels} (on the order of 1--4\%).
...
Specificity shows a non-uniform behavior across configurations.
Connecting GNN blocks to the first one or two MELD feature stages results in a substantial increase in specificity (approximately +18 and +9 percentage points, respectively).
...
Compared to the no-GNN baseline (0 layers), this corresponds to an absolute improvement of $\sim$10.4 percentage points in Dice and $\sim$8.3 percentage points in IoU, highlighting the benefit of incorporating the proposed GNN block.
