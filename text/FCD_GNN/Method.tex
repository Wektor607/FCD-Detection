...
To aggregate higher-order geometric information, we process the \textbf{6 feature layers} individually through a dedicated \textbf{GNN Block}.  
...
discriminative information, while the top 6 layers yielded the best trade-off between expressivity and stability.  
...
which is specifically designed for radiology reports. 
Prior studies have suggested that selectively unfreezing the last few layers of transformer models 
can improve downstream performance compared to freezing the entire model~\cite{Peters2019ToTune}. 
In our implementation, RadBERT is primarily used in a frozen setting, while the potential effect of 
layer unfreezing is further examined in Chapter~\ref{chapter:Experiments}. 
The output of the final hidden layer is forwarded to each GuideDecoder for multimodal fusion, 
except for the topmost three layers.
...
we employ a custom \texttt{SpiralConv}-based block that directly operates on the cortical surface mesh 
