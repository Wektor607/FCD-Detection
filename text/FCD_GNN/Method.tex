...
This design choice is based on empirical findings (see Chapter~\ref{chapter:Experiments}), where incorporating the lowest-resolution layers led to oversmoothing and degradation of discriminative information, while using only the higher-resolution layers provided the best trade-off between expressivity and stability.
...
level, producing a single subject-level prediction $\hat{c}$.
...
At each level $i$, auxiliary predictions are generated, and cross-entropy, Dice,
and distance losses are applied to these outputs.
