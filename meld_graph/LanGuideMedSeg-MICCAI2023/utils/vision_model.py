...
class NodeFeatureMask:
    """С вероятностью p зануляем случайные элементы признакового вектора каждой вершины."""
...
class EdgeDrop:
    """С вероятностью p удаляем каждую из рёбер (т.е. строку в edge_index)."""
    def __init__(self, p: float):
        assert 0 <= p <= 1
        self.p = p
...
class VisionModel(nn.Module):
    def __init__(
        ...
        self.node_aug = NodeFeatureMask(p=0.1)
        self.edge_aug = EdgeDrop(p=0.2)
    ...
    def _collect_edge_indices(self) -> Dict[str, Tuple[int, torch.Tensor]]:
        ...
        input_root = os.path.join(self.feature_path, "input", "ds004199")
        ...
        if not os.path.isfile(os.path.join(self.output_dir, "predictions_reports", f"{example_subj}", "predictions/prediction.nii.gz")) or \
            not os.path.isfile(example_npz):
    ...
    def forward(self, subject_ids: List[str]):
        ...
        for subject_id in subject_ids:
            ...
            if not os.path.isfile(os.path.join(self.output_dir, "predictions_reports", f"{subject_id}", "predictions/prediction.nii.gz")) or \
            ...
            for i, stage in enumerate(sorted_keys):
                ...
                data = Data(x=feat_tensor, 
                            ...
                            num_nodes=H * N)
                # Augmentation
                # data = self.node_aug(data)
                # data = self.edge_aug(data)
