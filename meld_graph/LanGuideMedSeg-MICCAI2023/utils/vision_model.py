...
class VisionModel(nn.Module):
    def __init__(
        ...
        self._nverts_to_level = {
            len(self.icos.icospheres[l]['coords']): l
            for l in self.icos.icospheres
        }
    ...
    def _collect_edge_indices(self) -> Dict[str, Tuple[int, torch.Tensor]]:
        ...
        input_root = os.path.join(self.feature_path, 'input', 'data4sharing', 'meld_combats')
        ...
        example_npz = os.path.join(self.feature_path, 'preprocessed', 'meld_files', example_subj, 'features', 'feature_maps.npz')
        ...
        for st in all_stage_keys:
            ...
            # найдём, на каком уровне icosphere именно N_i вершин
            if N_i not in self._nverts_to_level:
                raise ValueError(f'Не найден level для N_i={N_i}')
            level = self._nverts_to_level[N_i]

            # t_edges — это [2, E] для одного «полушария»
            t_edges = self.icos.icospheres[level]['t_edges']  # LongTensor

            # дублируем для правого полушария, сдвигая индексы на N_i
            edge_lh = t_edges
    ...
    def forward(self, subject_ids: List[str]):
        ...
        for subject_id in subject_ids:
            ...
            features_dir = os.path.join(self.feature_path, 'preprocessed', 'meld_files', subject_id, 'features')
